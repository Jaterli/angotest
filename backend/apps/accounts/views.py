...
from .serializers import (
    ...
    UpdateEmailPasswordSerializer, UpdateGuestProfileSerializer,
...
class LoginView(APIView):
    ...
    # Hash "dummy" precalculado en el arranque del proceso: se usa para que
    # check_password() siempre se ejecute con un coste similar aunque el
    # email no exista, y así evitar que el tiempo de respuesta revele si
    # una cuenta existe (timing side-channel).
    _DUMMY_HASH = make_password(secrets.token_hex(16))
    ...
    def post(self, request):
        ...
        if user is None:
            # Ejecutamos check_password igualmente contra un hash dummy
            # para mantener un tiempo de respuesta similar al caso real.
...
class ForgotPasswordView(APIView):
    ...
    def post(self, request):
        ...
        # No se loguea el token/enlace completo: filtrar el log expondría
...
@extend_schema(
    ...
    request=UpdateEmailPasswordSerializer,
    responses={
...
class UpdateEmailPasswordView(APIView):
    ...
    def post(self, request):
        user = request.user
        serializer = UpdateEmailPasswordSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
...
@extend_schema(
    ...
    request=UpdateGuestProfileSerializer,
    responses={
...
class UpdateGuestProfileView(APIView):
    ...
    def post(self, request):
        ...
        serializer = UpdateGuestProfileSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
...
class DeactivateAccountView(APIView):
    ...
    def delete(self, request):
        ...
        try:
            UserManagementService.anonymize_and_transfer(user)
        except ValueError as e:
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
        except ContainerUserError as e:
...
class AdminUserListView(ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if page is not None:
            ...
            # get_paginated_response siempre devuelve un dict (results/count/...)
            response.data['stats'] = stats
            return response

        # Paginación desactivada: serializer.data es una lista, no un dict,
        # así que envolvemos manualmente para poder incluir 'stats'.
...
class AdminDeleteUserView(DestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        try:
            result = UserManagementService.transfer_and_delete(user)
        except ValueError as e:
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
        except ContainerUserError as e:
            return Response(e.payload, status=status.HTTP_400_BAD_REQUEST)
